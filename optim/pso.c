...
static int compare_double(const void *a, const void *b) {
  ...
  if (a_nan && !b_nan) return 1;   // NaNs last
...
static inline double median_of_sorted(double *buf, int n) {
  return (n & 1) ? buf[n/2] : 0.5 * (buf[n/2 - 1] + buf[n/2]);
}

static void median_and_mad_relative(const double *v, int n, double best,
                                    double *median_r, double *mad_r) {
  *median_r = 0.0; *mad_r = 0.0;
  if (n <= 0) return;
  const double denom = fabs(best) + 1e-12;
  // buffer: percentage deviation from best. value - best / best
  ...
  // median
  qsort(buf, (size_t)m, sizeof(double), compare_double);
  double med = median_of_sorted(buf, m);
  // MAD
  for (int i = 0; i < m; ++i) buf[i] = fabs(buf[i] - med);
  qsort(buf, (size_t)m, sizeof(double), compare_double);
  double md = median_of_sorted(buf, m);
...
// Public stats: robust location & spread on *relative* scale
static void stats(const double *v, int n, double *location, double *spread) {
  if (n <= 0) { *location = 0.0; *spread = 0.0; return; }
  double best = v[0];
  for (int i = 1; i < n; ++i) if (v[i] < best) best = v[i];
  double med_r, mad_r;
  median_and_mad_relative(v, n, best, &med_r, &mad_r);
  *location = med_r;
  *spread   = 1.4826 * mad_r;
}
...
void pso(double* lb, double* ub,
  ...
  while (iter < ngen) {
    ...
    iter++;
    double location, spread;
    stats(s->swarm_bests, npop, &location, &spread);
    const double ratio = fabs(location) / (spread + 1e-12);
    printf("Generation: %d/%d, global best: %.6f; rel location/spread: %.6f / %.6f; Ratio: %.3f \n",
           iter, ngen, global_best_error, location, spread, ratio);
    if (global_best_error <= error_threshold) break;
    if ((ratio > 1.5) || (no_improvement == 10)) {
